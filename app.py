...
def tela_login():
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    section[data-testid="stMain"] {
        background: radial-gradient(ellipse at 20% 80%, rgba(180,20,50,0.35) 0%, transparent 55%),
                    radial-gradient(ellipse at 80% 20%, rgba(100,10,30,0.25) 0%, transparent 55%),
                    linear-gradient(160deg, #0f0a0f 0%, #1a0818 35%, #120a18 65%, #0a0f18 100%) !important;
        min-height: 100vh;
    }
    .block-container { padding: 0 !important; max-width: 100% !important; }
    .login-wrap {
        display: flex; align-items: center; justify-content: center;
        min-height: 95vh; padding: 24px;
        font-family: 'Poppins', sans-serif;
    }
    .login-wrap::before {
        content: '';
        position: fixed; inset: 0;
        background-image: linear-gradient(rgba(200,50,70,0.025) 1px, transparent 1px),
                          linear-gradient(90deg, rgba(200,50,70,0.025) 1px, transparent 1px);
    ...
    .lc-label { font-size: 9px; font-weight: 700; color: rgba(255,255,255,0.28); letter-spacing: 2px; text-transform: uppercase; margin-bottom: 8px; }
    .lc-input { width: 100%; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; padding: 12px 16px; color: white; font-family: 'Poppins', sans-serif; font-size: 15px; letter-spacing: 3px; outline: none; transition: all 0.2s; margin-bottom: 12px; }
    .lc-input:focus { border-color: rgba(210,45,65,0.5); background: rgba(210,45,65,0.04); }
    .lc-input::placeholder { letter-spacing: 2px; color: rgba(255,255,255,0.12); }
    .lc-btn { width: 100%; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.09); border-radius: 10px; padding: 12px; color: rgba(255,255,255,0.28); font-family: 'Poppins', sans-serif; font-size: 11px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; cursor: pointer; transition: all 0.3s; display: flex; align-items: center; justify-content: center; gap: 8px; }
    .lc-btn.on { background: #c8253f; border-color: #c8253f; color: white; }
    .lc-btn.on:hover { background: #a81e34; }
    .lc-err { background: rgba(210,45,65,0.08); border: 1px solid rgba(210,45,65,0.25); border-radius: 8px; padding: 10px 14px; color: #e05070; font-size: 11px; font-weight: 500; margin-top: 10px; display: none; }
    ...
    div[data-testid="stTextInput"] { position: absolute !important; opacity: 0 !important; pointer-events: none !important; height: 0 !important; overflow: hidden !important; }
    </style>
    ...
    <div class="login-wrap">
      <div class="lc">
        ...
        <input class="lc-input" id="lc-pwd" type="password" placeholder="••••••••••"
          oninput="document.getElementById('lc-btn').className='lc-btn'+(this.value?' on':'')"
          onkeydown="if(event.key==='Enter')submitPwd()"/>
        <button class="lc-btn" id="lc-btn" onclick="submitPwd()">
    ...
    if senha:
        if hashlib.md5(senha.encode()).hexdigest() == APP_PASSWORD_HASH:
            ...
            st.markdown("""<script>
            var e = document.getElementById('lc-err');
